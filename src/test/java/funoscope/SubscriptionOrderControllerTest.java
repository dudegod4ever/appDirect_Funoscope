...
public class SubscriptionOrderControllerTest {
    ...
    public void testSubscriptionOrder() throws Exception {
        ...
        String tempUrl2 = OauthUtil.signUrl(url, WebConstants.FUNOSCOPE_CONSUMER_KEY_VALUE, WebConstants.FUNOSCOPE_SECRET_KEY_VALUE);
        OauthContext context = new OauthContext();
        context.populateFromUrl(tempUrl);
        String authHeaderValue = context.getHeader();
        ...
        MvcResult subscriptionResult = mockMvc
                                              .perform(MockMvcRequestBuilders.get(path).accept(MediaType.APPLICATION_XML_VALUE)
                                                                             .header(WebConstants.AUTHORIZATION, authHeaderValue).param(WebConstants.URL, url))
    ...
    @Test
    public void testSubscriptionOrderStub() throws Exception {
        String path = WebConstants.URL_APPDIRECT_OAUTH_VALIDATION_STUB.replace(WebConstants.PARAM_VERSION, VERSION_V1).replace(WebConstants.PARAM_EVENT_TOKEN,
                                                                                                                               EVENT_TOKEN);

        MvcResult subscriptionResult = mockMvc.perform(MockMvcRequestBuilders.get(path).accept(MediaType.APPLICATION_XML_VALUE))
                                              .andExpect(MockMvcResultMatchers.status().isOk()).andReturn();
        assertNotNull(subscriptionResult);
        String resultString = subscriptionResult.getResponse().getContentAsString();
        assertNotNull(resultString);
